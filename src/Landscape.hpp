...
class HeightMap {
public:
  HeightMap(unsigned int seed, size_t n, float topLeft, float topRight, float bottomLeft,
            float bottomRight);
  ...
  float heightMin;
private:
  void diamondSquare(size_t n, float range, unsigned int seed, glm::ivec2 tl, glm::ivec2 tr,
                     glm::ivec2 bl, glm::ivec2 br);

  void safeSquareStep(glm::ivec2 target, glm::ivec2 l, glm::ivec2 u, glm::ivec2 r, glm::ivec2 d,
                      float randVal);
};

class LandscapeModel : public Drawable {
public:
  LandscapeModel(std::vector<float> heights, float heightMin, float heightMax, unsigned int seed,
                 size_t width, glm::uvec2 buildSiteCenter, std::vector<glm::uvec2> doodad,
                 std::shared_ptr<Shader> shader, bool flatten = false);
  void draw();
